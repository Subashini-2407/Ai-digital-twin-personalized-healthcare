            ...
            with col_r2:
                ...
                <div style='background: #f8f9fa; padding: 2rem; border-radius: 20px; height: 100%;'>
                    ...
                    <table style='width:100%'>
                        <tr><td>Age: </td><td><b>{age} years</b></td></tr>
                        <tr><td>BMI: </td><td><b>{bmi:.1f}</b></td></tr>
                        <tr><td>Blood Pressure: </td><td><b>{sbp} mmHg</b></td></tr>
                        <tr><td>Cholesterol: </td><td><b>{cholesterol} mg/dL</b></td></tr>
                        <tr><td>Smoker: </td><td><b>{'Yes' if smoker else 'No'}</b></td></tr>
                    </table>
                </div>
                """, unsafe_allow_html=True)
...
- Type **'new'** to start a new assessment"""
    ...
    # ==================== FEEDBACK & REVIEW PAGE ====================
        ...
        <div style='background: linear-gradient(135deg, #667eea10 0%, #764ba210 100%); 
                    padding: 1.5rem; border-radius: 15px; margin-bottom: 2rem;
                    border-left: 5px solid #1E88E5;'>
            <h4 style='margin:0; color: #1E88E5;'>💬 Help Us Improve</h4>
            <p style='margin:0.5rem 0 0 0; color: #666;'>
                Your feedback helps us make this app better. Thank you for taking the time to share your thoughts!
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            reviewer_name = st.text_input("Your Name (Optional)", placeholder="Enter your name")
            ...
            feedback_text = st.text_area(
                "Your Feedback",
                placeholder="What do you like about this app? What can we improve?",
                height=150
            )
            ...
            if st.button("📤 Submit Feedback", type="primary", use_container_width=True):
                if feedback_text.strip() or suggestion.strip():
                    new_feedback = {
                    ...
                    st.session_state.feedback_list.append(new_feedback)
                    ...
                    if "⭐⭐⭐⭐⭐" in rating:
                        ...
                        <div style='background: #43A04720; padding: 1rem; border-radius: 15px; text-align: center;'>
                            <h3 style='color: #43A047;'>🎉 Thank you for 5 stars!</h3>
                            <p>You're awesome! ⭐</p>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            sample_feedbacks = [
                {"name": "Sarah M.", "rating": "⭐⭐⭐⭐⭐", "feedback": "Amazing app! Helped me understand my health risks better. The what-if simulator is my favorite feature.", "date": "2 days ago"},
                {"name": "John D.", "rating": "⭐⭐⭐⭐", "feedback": "Very useful tool. Would love to see more diet recommendations.", "date": "5 days ago"},
                {"name": "Priya K.", "rating": "⭐⭐⭐⭐⭐", "feedback": "Life changing! Started exercising after seeing my risk score.", "date": "1 week ago"},
            ]
            ...
            for fb in sample_feedbacks:
                ...
                <div style='background: #f8f9fa; padding: 1rem; border-radius: 15px; margin-bottom: 1rem;
                            border-left: 4px solid #1E88E5;'>
                    <div style='display: flex; justify-content: space-between; align-items: center;'>
                        <div style='color: #FFD700; font-size: 1rem;'>{fb['rating']}</div>
                        <div style='font-size: 0.7rem; color: #999;'>{fb['date']}</div>
                    </div>
                    <p style='margin: 0.5rem 0; color: #444;'>"{fb['feedback']}"</p>
                    <div style='font-size: 0.8rem; color: #666;'>- {fb['name']}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        if st.session_state.feedback_list:
            ...
            for fb in st.session_state.feedback_list[-3:]:
                st.markdown(f"""
                <div style='background: #f0f7ff; padding: 0.8rem; border-radius: 10px; margin-bottom: 0.5rem;
                            border-left: 3px solid #1E88E5;'>
                    <div style='display: flex; justify-content: space-between;'>
                        <div style='color: #FFD700;'>{fb['rating']}</div>
                        <div style='font-size: 0.7rem; color: #999;'>{fb['date']}</div>
                    </div>
                    <p style='margin: 0.5rem 0 0 0; font-size: 0.9rem;'>{fb['feedback'][:150]}...</p>
                </div>
                """, unsafe_allow_html=True)
        ...
        if st.session_state.feedback_list:
            ...
            if st.button("📥 Export My Feedback Data", use_container_width=True):
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name=f"my_feedback_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv"
                )
