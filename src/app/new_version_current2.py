...
if 'kf_engine' not in st.session_state:
    st.session_state.kf_engine = None
...
if authenticated:
    ...
    with st.sidebar:
        ...
        # Navigation options - Updated with Family Health and Wellness Challenges
        nav_options = ["Executive Dashboard", "Risk Analysis", "Multi-Task Risk Predictor", "Lifestyle Optimizer", 
                      ...
                      "Mobile Connection", "Twin Analysis", 
                      "Sleep Wellness", "Health Monitor", "Vital Signs Tracker",
                      "Health Recommendations", "Task Scheduler", "Care Adherence",
                      "AI Health Chat", "Personal Assistant", "Family Health", "Wellness Challenges", "Settings"]
        ...
        if option_menu:
            selected = option_menu(
                ...
                icons=["house", "shield", "activity", "heart", "cpu", "graph-up", 
                       ...
                       "stars", "list-task", "graph-up", "chat-dots", "person-badge", 
                       "people", "trophy", "gear"],
                menu_icon="cast",
                default_index=nav_options.index(st.session_state.selected) if st.session_state.selected in nav_options else 0,
                styles={
            ...
            selected = st.radio("Navigation", nav_options, index=nav_options.index(st.session_state.selected) if st.session_state.selected in nav_options else 0)
    ...
    # ==================== TWIN ANALYSIS SECTION ====================
        ...
        if not st.session_state.twin_model_loaded:
            ...
            if st.button("🔍 Analyze Twin Health Risks", type="primary", width='stretch'):
                twin_a_data = {
    ...
    # ==================== NEW SECTION: SLEEP WELLNESS ====================
    elif selected == "Sleep Wellness":
        st.markdown("## 😴 Sleep Wellness Analysis")
        st.info("Enter your sleep data for the last 7 days to detect potential sleep disorders.")
        
        days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
        sleep_data = []
        for i, day in enumerate(days):
            col1, col2, col3, col4 = st.columns(4)
            with col1: hours = st.number_input(f"{day} hours", 3.0, 12.0, 7.0, 0.5, key=f"hr_{i}")
            with col2: quality = st.slider(f"{day} quality", 1, 10, 7, key=f"ql_{i}")
            with col3: deep = st.slider(f"{day} deep%", 10, 40, 20, key=f"dp_{i}")
            with col4: rem = st.slider(f"{day} rem%", 15, 30, 22, key=f"rm_{i}")
            sleep_data.append([hours, quality, deep, rem])
        
        if st.button("🔍 Analyze Sleep Patterns", type="primary", width='stretch'):
            # Simple analysis for demo
            avg_quality = np.mean([d[1] for d in sleep_data])
            avg_hours = np.mean([d[0] for d in sleep_data])
            
            if avg_quality < 5 or avg_hours < 6:
                disorder = True
                prob = 0.75 + (5 - avg_quality) * 0.05
            else:
                disorder = False
                prob = 0.25 + (avg_quality - 5) * 0.05
            
            prob = min(0.95, max(0.05, prob))
            
            col_r1, col_r2 = st.columns(2)
            with col_r1:
                fig = px.imshow(np.array(sleep_data)[:, :2].T, x=days, y=['Hours', 'Quality'], title="Sleep Trends (Last 7 Days)")
                st.plotly_chart(fig, width='stretch')
            with col_r2:
                if disorder:
                    st.error(f"⚠️ High probability of sleep disorder detected! ({prob*100:.1f}%)")
                    st.markdown("**📋 Recommendations:**")
                    st.markdown("- Maintain consistent bedtime (same time every night)")
                    st.markdown("- Avoid screens 1 hour before bed")
                    st.markdown("- Limit caffeine after 2 PM")
                    st.markdown("- Try meditation before sleep")
                else:
                    st.success(f"✅ Sleep pattern looks normal (disorder probability: {prob*100:.1f}%)")
                    st.markdown("**💡 Tips to maintain healthy sleep:**")
                    st.markdown("- Keep your bedroom dark and cool")
                    st.markdown("- Stick to your sleep schedule even on weekends")
                    st.markdown("- Get morning sunlight exposure")
            
            sleep_score = 100 - (prob * 100)
            fig_gauge = go.Figure(go.Indicator(
                mode="gauge+number",
                value=sleep_score,
                title={'text': "Sleep Health Score"},
                domain={'x': [0, 1], 'y': [0, 1]},
    ...
    # ==================== NEW SECTION: HEALTH MONITOR ====================
    elif selected == "Health Monitor":
        st.markdown("## 🚨 Health Monitor")
        st.markdown("""<div style='background:linear-gradient(135deg,#667eea10 0%,#764ba210 100%);padding:1.5rem;border-radius:15px;margin-bottom:2rem;border-left:5px solid #1E88E5;'><h4 style='margin:0;color:#1E88E5;'>🚨 Real-time Health Monitoring</h4><p style='margin:0.5rem 0 0 0;color:#666;'>Monitor your vital signs and receive alerts for abnormal patterns.</p></div>""", unsafe_allow_html=True)
        
        col1, col2, col3 = st.columns(3)
        with col1: current_hr = st.number_input("Heart Rate (BPM)", 40, 150, 75)
        with col2: current_bp = st.number_input("Blood Pressure (systolic)", 80, 200, 120)
        with col3: current_temp = st.number_input("Temperature (°C)", 35.0, 40.0, 36.6, 0.1)
        ...
        if st.button("🔍 Run Health Check", type="primary", width='stretch'):
            ...
            if current_hr > 100:
                is_anomaly = True
                score += 0.4
                alerts.append(f"⚠️ High heart rate: {current_hr} BPM")
            if current_hr < 50:
                is_anomaly = True
                score += 0.3
                alerts.append(f"⚠️ Low heart rate: {current_hr} BPM")
            if current_bp > 140:
                is_anomaly = True
                score += 0.4
                alerts.append(f"⚠️ High blood pressure: {current_bp} mmHg")
            if current_bp < 90:
                is_anomaly = True
                score += 0.3
                alerts.append(f"⚠️ Low blood pressure: {current_bp} mmHg")
            if current_temp > 37.5:
                is_anomaly = True
                score += 0.3
                alerts.append(f"⚠️ Elevated temperature: {current_temp}°C")
            ...
            with col_r1:
                if is_anomaly:
                    st.error(f"🚨 Health Alert Detected!")
                    st.metric("Alert Score", f"{score:.2f}", delta="Action Needed", delta_color="inverse")
                    for alert in alerts:
                        st.warning(alert)
                else:
                    st.success(f"✅ All vitals normal")
                    st.metric("Health Score", f"{100 - score*100:.0f}%", delta="Good", delta_color="normal")
    ...
    # ==================== NEW SECTION: VITAL SIGNS TRACKER ====================
    elif selected == "Vital Signs Tracker":
        st.markdown("## 💓 Vital Signs Tracker")
        st.markdown("""<div style='background:linear-gradient(135deg,#667eea10 0%,#764ba210 100%);padding:1.5rem;border-radius:15px;margin-bottom:2rem;border-left:5px solid #1E88E5;'><h4 style='margin:0;color:#1E88E5;'>💓 Smart Vital Signs Tracking</h4><p style='margin:0.5rem 0 0 0;color:#666;'>Track your heart rate with advanced signal smoothing for accurate monitoring.</p></div>""", unsafe_allow_html=True)
        ...
        class SmoothingFilter:
            ...
            def update(self, measurement):
                if self.value is None:
                    ...
                    self.value = self.value * (1 - self.k) + measurement * self.k
                return self.value
        
        if 'smoothing_filter' not in st.session_state:
            st.session_state.smoothing_filter = SmoothingFilter()
        if 'vital_hr_raw' not in st.session_state:
            st.session_state.vital_hr_raw = []
        if 'vital_hr_smoothed' not in st.session_state:
            st.session_state.vital_hr_smoothed = []
        
        col1, col2 = st.columns(2)
        with col1:
            new_hr = st.number_input("Heart Rate (BPM)", 40, 150, 75)
            if st.button("➕ Add Reading", width='stretch'):
                smoothed = st.session_state.smoothing_filter.update(new_hr)
                st.session_state.vital_hr_raw.append(new_hr)
                st.session_state.vital_hr_smoothed.append(smoothed)
                st.rerun()
        with col2:
            if st.button("🎮 Simulate Exercise Pattern", width='stretch'):
                pattern = [70, 72, 68, 95, 110, 115, 105, 90, 85, 82, 78, 75]
                for v in pattern:
                    smoothed = st.session_state.smoothing_filter.update(v)
                    st.session_state.vital_hr_raw.append(v)
                    st.session_state.vital_hr_smoothed.append(smoothed)
                st.rerun()
        
        if len(st.session_state.vital_hr_raw) > 0:
            st.markdown("---")
            st.markdown("### 📊 Heart Rate Data")
            
            df_hr = pd.DataFrame({
                'Reading': range(1, len(st.session_state.vital_hr_raw) + 1),
                'Raw (BPM)': st.session_state.vital_hr_raw,
                'Smoothed (BPM)': st.session_state.vital_hr_smoothed
            })
            
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df_hr['Reading'], y=df_hr['Raw (BPM)'], 
                                    mode='lines+markers', name='Raw Reading',
                                    line=dict(color='#9E9E9E', width=1, dash='dot'),
                                    marker=dict(size=6, color='#9E9E9E')))
            fig.add_trace(go.Scatter(x=df_hr['Reading'], y=df_hr['Smoothed (BPM)'], 
                                    mode='lines+markers', name='Smoothed Signal',
            ...
            fig.update_layout(title="Heart Rate with Signal Smoothing",
                             xaxis_title="Reading Number", yaxis_title="Heart Rate (BPM)",
                             height=400, hovermode='x unified')
            st.plotly_chart(fig, width='stretch')
            
            col_m1, col_m2 = st.columns(2)
            with col_m1: st.metric("Latest Raw", f"{st.session_state.vital_hr_raw[-1]} BPM")
            with col_m2: st.metric("Latest Smoothed", f"{st.session_state.vital_hr_smoothed[-1]:.1f} BPM")
            ...
            st.info("No readings yet. Add a reading or simulate an exercise pattern to see the tracker in action.")

    # ==================== NEW SECTION: HEALTH RECOMMENDATIONS ====================
    elif selected == "Health Recommendations":
        st.markdown("## 🎯 Personalized Health Recommendations")
        st.markdown("""<div style='background:linear-gradient(135deg,#667eea10 0%,#764ba210 100%);padding:1.5rem;border-radius:15px;margin-bottom:2rem;border-left:5px solid #1E88E5;'><h4 style='margin:0;color:#1E88E5;'>🎯 AI-Powered Recommendations</h4><p style='margin:0.5rem 0 0 0;color:#666;'>Personalized health suggestions based on your profile.</p></div>""", unsafe_allow_html=True)
        
        user_history_rec = db.get_user_history(user_id, limit=1)
        if user_history_rec:
            last_pred_rec = user_history_rec[0]
            user_bmi_rec = last_pred_rec.get('bmi', 25)
            
            if user_bmi_rec > 28:
                recommendations_rec = [
                    {"title": "🏃 30-Minute Daily Walk", "impact": "Reduces CVD risk by 15%", "match": 95},
                    {"title": "🥗 Mediterranean Diet Plan", "impact": "Reduces heart disease risk by 30%", "match": 92},
                    {"title": "💧 Hydration Tracker", "impact": "Improves metabolism by 15%", "match": 88}
                ]
            elif user_bmi_rec > 25:
                recommendations_rec = [
                    {"title": "🚶‍♂️ 10,000 Steps Daily", "impact": "Improves cardiovascular health", "match": 90},
                    {"title": "🥬 Plant-Based Meals", "impact": "Lowers cholesterol naturally", "match": 87},
                    {"title": "💪 Strength Training", "impact": "Builds muscle & burns fat", "match": 85}
                ]
            else:
                recommendations_rec = [
                    {"title": "🏋️ Strength Training", "impact": "Builds muscle & bone density", "match": 90},
                    {"title": "😴 Sleep Schedule Optimization", "impact": "Improves sleep quality by 40%", "match": 87},
                    {"title": "🧘 Daily Meditation", "impact": "Reduces stress by 35%", "match": 85}
                ]
        else:
            recommendations_rec = [
                {"title": "🏃 30-Minute Daily Walk", "impact": "Reduces CVD risk by 15%", "match": 85},
                {"title": "🥗 Mediterranean Diet", "impact": "Reduces heart disease risk by 30%", "match": 82},
                {"title": "😴 Sleep Schedule Optimization", "impact": "Improves sleep quality by 40%", "match": 78}
            ]
        
        st.markdown("### 📋 Your Top Recommendations")
        for i, rec in enumerate(recommendations_rec, 1):
            match_pct = rec['match']
            if match_pct > 90:
                badge = "badge-success"
                icon = "🔥"
            ...
            st.markdown(f"""
            <div style='background:white;padding:1rem;border-radius:15px;margin-bottom:1rem;border-left:5px solid #1E88E5;'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div>
                        <span style='font-size:1.5rem;margin-right:0.5rem;'>{icon}</span>
                        <strong style='font-size:1.1rem;'>{i}. {rec['title']}</strong>
                    </div>
                    <span class='badge {badge}' style='font-size:0.9rem;'>Match: {match_pct}%</span>
                </div>
                <p style='margin:0.5rem 0 0 0;color:#666;font-size:0.9rem;'>{rec['impact']}</p>
            </div>
            """, unsafe_allow_html=True)
        
        st.info("These recommendations are generated based on users with similar health profiles.")

    # ==================== NEW SECTION: TASK SCHEDULER ====================
    elif selected == "Task Scheduler":
        st.markdown("## 📋 Smart Task Scheduler")
        st.markdown("""<div style='background:linear-gradient(135deg,#667eea10 0%,#764ba210 100%);padding:1.5rem;border-radius:15px;margin-bottom:2rem;border-left:5px solid #1E88E5;'><h4 style='margin:0;color:#1E88E5;'>📋 Priority-Based Task Management</h4><p style='margin:0.5rem 0 0 0;color:#666;'>Tasks are prioritized based on urgency for efficient management.</p></div>""", unsafe_allow_html=True)
        ...
        with st.form("new_task_scheduler"):
            task_name = st.text_input("Task name")
            col1, col2 = st.columns(2)
            with col1: urgency = st.slider("Urgency (0-100)", 0, 100, 50)
            with col2: due_date = st.selectbox("Due", ["Today", "Tomorrow", "This Week", "Next Week"])
            if st.form_submit_button("➕ Add Task", width='stretch'):
                st.session_state.task_list.append({"task": task_name, "urgency": urgency, "due": due_date})
                st.success("Task added to priority queue!")
                st.rerun()
        
        # Sort by urgency (highest first)
        sorted_tasks = sorted(st.session_state.task_list, key=lambda x: x['urgency'], reverse=True)
        
        st.markdown("### 📊 Priority Queue (Highest First)")
        for i, task in enumerate(sorted_tasks, 1):
            ...
            st.markdown(f"""
            <div style='background:white;padding:1rem;border-radius:10px;margin-bottom:0.5rem;border-left:5px solid {color};'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div>
                        <span style='font-size:1.2rem;'>{icon}</span>
                        <strong>{i}. {task['task']}</strong>
                    </div>
                    <span class='badge {badge}' style='font-size:0.9rem;'>Urgency: {urgency}%</span>
                </div>
                <div style='font-size:0.85rem;color:#666;margin-top:0.3rem;'>Due: {task['due']}</div>
            </div>
            """, unsafe_allow_html=True)

    # ==================== NEW SECTION: CARE ADHERENCE ====================
    elif selected == "Care Adherence":
        st.markdown("## 📊 Care Adherence Prediction")
        st.markdown("""<div style='background:linear-gradient(135deg,#667eea10 0%,#764ba210 100%);padding:1.5rem;border-radius:15px;margin-bottom:2rem;border-left:5px solid #1E88E5;'><h4 style='margin:0;color:#1E88E5;'>📊 Treatment Adherence Forecasting</h4><p style='margin:0.5rem 0 0 0;color:#666;'>Predict patient adherence to treatment plans based on historical patterns.</p></div>""", unsafe_allow_html=True)
        ...
        if st.button("📊 Predict Adherence", type="primary", width='stretch'):
            ...
            with col_r1:
                ...
                <div style='text-align:center;padding:2rem;background:white;border-radius:20px;'>
                    <div style='font-size:1rem;color:#666;'>Predicted Adherence Rate</div>
                    <div style='font-size:4rem;font-weight:700;color:#1E88E5;'>{prob*100:.1f}%</div>
                </div>
                """, unsafe_allow_html=True)
            
            with col_r2:
                if prob > 0.8:
                    st.success("✅ High likelihood of good adherence")
                    st.markdown("**Recommendations:**")
                    st.markdown("- Continue current support system")
                    st.markdown("- Monthly check-ins sufficient")
                elif prob > 0.5:
                    st.warning("⚠️ Moderate adherence predicted")
                    st.markdown("**Recommendations:**")
                    st.markdown("- Set up medication reminders")
                    st.markdown("- Weekly check-ins recommended")
                    st.markdown("- Simplify medication regimen if possible")
                else:
                    st.error("🔴 Low predicted adherence - Intervention needed")
                    st.markdown("**Recommendations:**")
                    st.markdown("- Immediate intervention required")
                    st.markdown("- Daily reminders and support")
                    st.markdown("- Consider pill organizer or smart dispenser")
                    st.markdown("- Schedule daily check-ins")
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=weeks, y=historical, mode='lines+markers', name='Historical', line=dict(color='#1E88E5', width=3)))
            fig.add_trace(go.Scatter(x=future_weeks, y=predicted_future, mode='lines+markers', name='Predicted', line=dict(color='#E53935', width=3, dash='dash')))
            fig.add_hline(y=80, line_dash="dash", line_color="green", annotation_text="Target 80%")
            fig.update_layout(title="Adherence Trend with Prediction", xaxis_title="Week", yaxis_title="Adherence Rate (%)", height=350)
            st.plotly_chart(fig, width='stretch')
    ...
    # ==================== AI HEALTH CHAT - CONVERSATIONAL ASSISTANT ====================
        ...
        questions = [
            ...
            ("bp", "What's your blood pressure (systolic/diastolic)? (e.g., 120/80)"),
            ("weight", "What's your weight in kg? (e.g., 75)"),
            ("exercise", "How many minutes of exercise do you get per week? (e.g., 150)"),
            ("diet", "How would you rate your diet? (poor/fair/good/excellent)"),
            ("sleep", "How many hours of sleep do you get per night? (e.g., 7)"),
            ("stress", "What's your stress level? (low/moderate/high)"),
        ...
        user_input = st.text_input(
            ...
            key=f"health_input_{st.session_state.health_chat_step}"
        )
        submitted = st.button("Send", use_container_width=True)

        if submitted and user_input.strip():
            current_step = st.session_state.health_chat_step
            st.session_state.chat_user_responses[questions[current_step][0]] = user_input
            ...
            if next_step < len(questions):
                ...
                try:
                    age = int(st.session_state.chat_user_responses.get('age', 50))
                    bp = int(st.session_state.chat_user_responses.get('bp', '120').split('/')[0])
                    weight = float(st.session_state.chat_user_responses.get('weight', 75))
                    exercise = int(st.session_state.chat_user_responses.get('exercise', 150))
                    smoking = st.session_state.chat_user_responses.get('smoking', 'no').lower() == 'yes'
                    ...
                    response = f"""Thank you! Based on your responses, here's your assessment:

**Your Health Risk Score: {risk_score:.1f}%**

Your personalized risk profile has been calculated. Here's what I recommend:
...
Would you like more detailed advice on any of these areas?"""
                except Exception as e:
                    response = f"I had trouble calculating your risk. Please try again with numeric values. Error: {str(e)}"
    ...
    # ==================== FAMILY HEALTH ====================
        ...
        <div style='background: linear-gradient(135deg, #667eea10 0%, #764ba210 100%); 
                    padding: 1.5rem; border-radius: 15px; margin-bottom: 2rem;
                    border-left: 5px solid #1E88E5;'>
            <h4 style='margin:0; color: #1E88E5;'>👪 Track Health for Your Entire Family</h4>
            <p style='margin:0.5rem 0 0 0; color: #666;'>
                Add family members, monitor their health risks, and receive family-wide insights.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            ...
            member_relation = st.selectbox("Relation", ["Spouse", "Child", "Parent", "Grandparent", "Sibling", "Other"])
            ...
            if st.button("➕ Add Member", type="primary", use_container_width=True):
                if member_name:
                    simulated_risk = 0.05 + (member_age / 200) + np.random.uniform(-0.05, 0.05)
                    simulated_risk = np.clip(simulated_risk, 0.02, 0.4)
                    simulated_health = int(100 - (simulated_risk * 100))
            ...
            if st.session_state.family_members:
                for idx, member in enumerate(st.session_state.family_members):
                    with st.expander(f"👤 {member['name']} ({member['relation']}, Age {member['age']})"):
                        col_a, col_b, col_c = st.columns(3)
                        with col_a:
                            st.metric("Health Score", f"{member['health_score']}/100")
                        with col_b:
                            st.metric("5-Year Risk", f"{member['risk_score']*100:.1f}%")
                        with col_c:
                            if st.button(f"Remove", key=f"remove_family_{idx}"):
                                st.session_state.family_members.pop(idx)
                                st.rerun()
                        
                        if member['risk_score'] < 0.1:
                            st.success("✅ Low risk profile")
                        elif member['risk_score'] < 0.2:
                            st.warning("⚠️ Moderate risk")
                        else:
                            st.error("🔴 High risk - Medical consultation advised")
            else:
                st.info("No family members added yet. Add family members to track their health!")
        ...
        if st.session_state.family_members:
            ...
            with col_s2:
                st.metric("Average Risk", f"{family_df['risk_score'].mean()*100:.1f}%")
            with col_s3:
                high_risk = len(family_df[family_df['risk_score'] > 0.2])
                st.metric("High Risk Members", high_risk)
            
            fig = px.bar(family_df, x='name', y='risk_score', title="Family Risk Comparison",
                        color='risk_score', color_continuous_scale='RdYlGn_r',
                        text=family_df['risk_score'].apply(lambda x: f"{x*100:.1f}%"))
